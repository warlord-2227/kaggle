...
SP = os.path.dirname(os.path.abspath(__file__)); ROOT = "/home/iwa/working/kaggle"; D = ROOT + "/replays/live26"; S = 56453495
d = json.load(open(f"{SP}/eps_{S}.json"))
...
for e in eps:
    ...
    if (op.get("initialScore") or 0) >= 2200:
...
print("games vs >=2200:", len(games), flush=True)
def fetch(g):
...
json.dump(rows, open(SP + "/live26_classified.json", "w"))
def band(r): return "2200-2400" if r < 2400 else ("2400-2500" if r < 2500 else "2500+")
print("\nkind × band: record (win rate), mean margin")
for kind in ("clone", "other"):
    for b in ("2200-2400", "2400-2500", "2500+"):
