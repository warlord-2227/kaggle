...
# Modified 2026-09-18: eight-turn sale advance experiment.
_ADV_LOOK=8

# Modified September 18, 2026 by Alperen Aydin's Kaggriculture project.
# Public-yield scenario quote-response over the credited V48 core.
# Rival shed quantities are scenarios, NOT recovered hidden observations.
_QR_PARENT=_e335_agent
_QR_ADAPTIVE=False
_QR_REPORT=dict(turns=0,changed=0,advance_changes=0,errors=0)

def _qr_supply(obs):
    rival=obs['farms'][1-int(obs['player'])]
    supply={p:0 for p in _E334_ITEMS}
    kinds={'COW':'MILK','SHEEP':'WOOL','GOOSE':'EGG'}
    for row in rival['tiles']:
        for tile in row:
            if not isinstance(tile,dict):continue
            item=kinds.get(tile.get('animal'),tile.get('crop'))
            if item in supply:supply[item]+=max(0,int(tile.get('yield_units',0) or 0))
    return supply

def _qr_orders(obs,action):
    orders=[list(o) for o in action.get('market',[])]
    if int(obs['step'])<144 or len(orders)<2:return action
    farm,private=_r127_fields(obs,action)
    stock={k:max(0,int(v)) for k,v in private['shed'].items()}
    visible=_qr_supply(obs);params=_v44y_params(obs);inventory=dict(obs['market']['inventory'])
    # Three explicitly declared plausible queues. Keep BUY/funding slots untouched.
    cash=[o for o in orders if len(o)>=3 and o[0]=='SELL' and o[1] in _E334_ITEMS]
    if len(cash)<2:return action
    rival_stock={p:min(32,max(4,visible.get(p,0),stock.get(p,0)//2)) for p in _E334_ITEMS}
    queues=[]
    for schedule in (cash,sorted(cash,key=lambda o:-params[o[1]]['base']),list(reversed(cash))):
        queues.append([['SELL',o[1],rival_stock[o[1]]] for o in schedule])
    cache={}
    def score(candidate):
        key=tuple(tuple(o) for o in candidate)
        if key not in cache:
            values=[]
            for rival_orders in queues:
                a,b=_v44y_lockstep(candidate,rival_orders,inventory,stock,rival_stock,params)
                values.append(a-0*b)
            cache[key]=statistics_mean(values)-0.25*(max(values)-min(values))
        return cache[key]
    best=orders;value=score(best);base=value
    # Bounded adjacent-exchange search, not factorial search on every action.
    for iteration in range(2):
        changed=False
        for i in range(len(best)-1):
            a,b=best[i:i+2]
            if not all(len(o)>=3 and o[0]=='SELL' and o[1] in _E334_ITEMS for o in (a,b)):continue
            trial=best[:];trial[i:i+2]=[b,a];v=score(trial)
            if v>value+0.5:best=trial;value=v;changed=True
        if not changed:break
    _QR_REPORT['turns']+=1
    if value<=base+0.5:return action
    _QR_REPORT['changed']+=1
...
def agent(observation,configuration=None):
    global _ADV_LOOK
    step=int(observation.get('step',0));saved=_ADV_LOOK
    if step==0:_QR_REPORT.update(turns=0,changed=0,advance_changes=0,errors=0)
    try:
        if _QR_ADAPTIVE and step>=144:
            supply=_qr_supply(observation)
            held=observation['private']['shed']
            # More visible competing yield for held goods warrants a longer race window.
            threat=sum(min(int(held.get(p,0)),q) for p,q in supply.items())
            _ADV_LOOK=24 if threat>=16 else 16 if threat>=4 else 8
            if _ADV_LOOK!=saved:_QR_REPORT['advance_changes']+=1
        result=_QR_PARENT(observation,configuration)
        standard=configuration is None or all(configuration.get(k,v)==v for k,v in [('boardSize',10),('turnsPerDay',24),('shedCapacity',100),('maxMarketOrdersPerTurn',10)])
        if standard:result=_qr_orders(observation,result)
        return result
    except Exception:
        _QR_REPORT['errors']+=1
        # Do not call the stateful parent twice on the same turn.
        return result if 'result' in locals() else {'farmer':['PASS'],'hands':[],'market':[]}
    finally:_ADV_LOOK=saved
agent.telemetry=_QR_REPORT
agent=globals().pop('agent')

# Modified 2026-09-19: declared revenue weight 0, window 24.
_ADV_LOOK=24

# Modified 2026-09-19: joint opening pressure10 and queue objective; preserve empty and funding slots.
_OPEN_ATTACK=10
